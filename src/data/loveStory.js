/** @format */

export const loveStoryEvents = [
	{
		date: 'The Beginning',
		title: 'Strangers to Someone Special',
		description:
			'Manam ela start ayyamo gurthu vasthe ippatiki navvostundi 😊 Stranger la start ayyamu, kani ippudu ninnu alochinchakunda oka roju kuda complete kaadhu 💝',
		icon: 'User',
	},
	{
		date: 'First Sight',
		title: 'Modati Sari Chusina Roju',
		description:
			'Modati sari ninnu chusina roju nunchi naaku edo different ga anipinchindi 💭 Adi crush aa, istam aa ani appudu teliyadu… kani prema ani ardham ayyindi 💖',
		icon: 'Eye',
	},
	{
		date: 'Little Secrets',
		title: 'Reasons to See You',
		description:
			'Nijam cheppalante, nenu chala sarlu ninnu chudataniki reasons create chesanu 😅💕 Nuvvu enjoy chestunte chusi silent ga happy ayyedani 💝',
		icon: 'Smile',
	},
	{
		date: 'Krishnaashtami',
		title: 'The Courageous Moment',
		description:
			'Naa heart lo unna matalu cheppadam naa life lo biggest courage moment 🙈💛 Bayam kuda undi… kani prema danikanna strong ga undi 💓',
		icon: 'Sparkles',
	},
	{
		date: 'The Bond',
		title: 'Our Safe Place',
		description:
			'Nuvvu accept chesina moment nunchi naku oka safe place dorikindi 🤗💝 Nuvvu naa life lo ochina best blessing ani nenu daily feel avthunna 🙏💛',
		icon: 'Home',
	},
	{
		date: 'My Promise',
		title: 'To You, Forever',
		description:
			'Nenu promise chesthunnanu… ninnu eppudu badha petanu 💞 Ninnu respect chesukunta, ardham chesukunta, santhosham ga chusukunta 😊',
		icon: 'Heart',
	},
	{
		date: 'Forever Us',
		title: 'Sweetest Chapter',
		description:
			'Na attention, na time, na care, na heart… anni neekey kanna 🫶💖 Stranger ga start ayina mana story… sweetest chapter ayyindi 📖💞 Love you so much Bujji Kanna 💝😘',
		icon: 'Star',
	},
];
